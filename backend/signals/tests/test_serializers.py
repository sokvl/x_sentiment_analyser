...
class SignalSerializerTests(TestCase):
    def setUp(self):
        ...
        self.signal = Signal.objects.create(
            ...
            confidence_score=0.85, used_model='LSTMCNNv1', config_ig=self.config,
        )
    ...
    def test_deserialization(self):
        data = {
            'signal_type': 'SELL',
            'ticker_id': self.ticker.ticker_id,
            'confidence_score': -0.5,
            'used_model': 'TransformerV2',
            'config_ig': self.config.config_id,
        }
